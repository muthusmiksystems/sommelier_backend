...
class PaymentList extends Component {
	...
	componentWillReceiveProps(nextProps) {
		...
		if (nextProps.checkout !== this.props.checkout) {
			...
			if (nextProps.checkout.data.payment_mode === "PAYMONGO") {
				if (this.state.paymongoNavigate !== "") {
					window.location = this.state.paymongoNavigate;
					return null;
				}
			}
			...
			if (nextProps.checkout.data.orderstatus_id !== 8) {
				...
				//Navigate to running order page
					...
					if (res.data.paymongo_success) {
						if (res.data.status === "succeeded") {
							...
							self.setState({
								...
								paymongoNavigate: res.data.Navigate_url,
							});
	...
	handlePaymentGatewayNavigate = (url) => {
		console.log("I am here");
		console.log("Placed order", this.props.checkout.id);
		const NavigateUrl = url + "&order_id=" + this.props.checkout.id;
											...
											{this.state.payWithStripeIdeal && (
												<div className="stripe-card-container">
													<StripeIdeal
														...
														handlePaymentGatewayNavigate={this.handlePaymentGatewayNavigate}
