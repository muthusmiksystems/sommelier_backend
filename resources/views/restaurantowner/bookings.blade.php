...
@section("title") {{__('storeDashboard.bpTitle')}}
@endsection
@section('content')
<style>
    ...
    <div class="page-header-content header-elements-md-inline">
        <div class="page-title d-flex">
            <h4><i class="icon-circle-right2 mr-2"></i>
                ...
                <span class="font-weight-bold ml-4">{{__('storeDashboard.bpNoOfPax')}}</span>
                <span class="badge badge-primary badge-pill animated flipInX">{{ $no_of_pax }}</span>
                @else
                <span class="font-weight-bold mr-2">{{__('storeDashboard.total')}}</span>
...
<div class="content">
    <form action="{{ route('restaurant.bookings') }}" method="GET">
        ...
        <input type="hidden" name="bookingdate" value="{{ (!empty($bookingdate)) ? $bookingdate : date('Y-m-d') }}"/>
        <input type="hidden" name="booking_status" value="{{ (!empty($booking_status)) ? $booking_status : 'all' }}"/>
        <!-- @csrf -->
    </form>
    <form action="{{ route('restaurant.bookings') }}" method="GET">
        <div class="row">
            ...
            <div class="col-lg-3">
                <select name="meal_type" class="form-control">
                    <option value="all" {{ ($meal_type == "all") ? 'selected="selected"' : "" }}>All Meal Type</option>
            ...
            <div class="col-lg-3">
                <div class="form-group form-group-feedback form-group-feedback-right search-box">
                    <select class="form-control" name="booking_status">
                        <option value="all" {{ ($booking_status == 'all' || $booking_status == '') ? "selected='selected'" : ''}}>All Booking Status</option>
                        <option value="open" {{ ($booking_status == 'open') ? "selected='selected'" : ''}}>Open</option>
                        <option value="reserved" {{ ($booking_status == 'reserved') ? "selected='selected'" : ''}}>Reserved</option>
                        <option value="completed" {{ ($booking_status == 'completed') ? "selected='selected'" : ''}}>Completed</option>
                        <option value="cancelled" {{ ($booking_status == 'cancelled') ? "selected='selected'" : ''}}>Cancelled</option>
                    </select>
                </div>
            </div>
            <div class="col-lg-3">
                <div class="form-group form-group-feedback form-group-feedback-right search-box">
                    <input type="date" class="form-control" placeholder="{{__('storeDashboard.bpBookingDate')}}"
                        name="bookingdate" value="{{ (!empty($bookingdate)) ? $bookingdate : date('Y-m-d') }}">
                </div>
            </div>
        </div>
        <div class="row">
            ...
            <div class="col-lg-3">
                <div class="form-group form-group-feedback form-group-feedback-right search-box">
                    <button type="submit" class="btn btn-secondary btn-labeled btn-labeled-left mr-2" name="action" id="BookingSearch" value="bookingSearch">
                        ...
                        {{__('storeDashboard.bpBookingSearch')}}
                    </button>
                    <button type="submit" class="btn btn-secondary btn-labeled btn-labeled-left mr-2" formaction="{{ route('restaurant.bookingsPrint') }}" name="action" id="BookingPrint" value="bookingPrint">
                        <b><i class="icon-printer"></i></b>
                        {{__('storeDashboard.bpBookingPrint')}}
                    </button>
                </div>
            </div>
        </div>
        ...
        <div class="card-body">
            <div class="table-responsive">
                <table class="table">
                    <thead>
                        <tr>
                            <th>{{__('storeDashboard.bpTableBookingId')}}</th>
                            <th>{{__('storeDashboard.bpTableCustomerName')}}</th>
                            <th>{{__('storeDashboard.bpTableShift')}}</th>
                            <th>{{__('storeDashboard.bpTableTableNo')}}</th>
                            <th>{{__('storeDashboard.bpTableTime')}}</th>
                            <th>{{__('storeDashboard.bpTablePax')}}</th>
                            <th>{{__('storeDashboard.bpTableComments')}}</th>
                            <th>{{__('storeDashboard.bpTableRestaurant')}}</th>
                            <th>
                                <div class="booking_action" style="display:flex;">
                                    <div class="action_title">
                                        {{__('storeDashboard.bpTableAction')}}
                                    </div>
                                    <div class="booking_done_all" style="margin-left:15px;">
                                        @if($in_complete_booking > 0 && strtotime(date('Y-m-d', strtotime($bookingdate))) < strtotime(date('Y-m-d')))
                                        
                                            <div class="checkbox checkbox-switchery ml-1">
                                                <label>
                                                <input value="true" type="checkbox" class="action-switch-booking-done-all" data-booking_date="{{ $bookingdate }}" data-booking_status="{{ $booking_status }}">
                                                </label>
                                            </div>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{ date('h:i a', strtotime($booking->booking_datetime)) }}</td>
                            ...
                            <td class="text-center">
                                <div class="btn-group btn-group-justified align-items-center">
                                    ...
                                    @if($booking->booking_status != 'completed' && $booking->booking_status != 'cancelled')
                                        <a href="{{ route('restaurant.get.editBooking', $booking->id) }}"
                                        class="badge badge-primary badge-icon"> {{__('storeDashboard.edit')}} <i
                                            class="icon-database-edit2 ml-1"></i></a>

                                        <a href="{{ route('restaurant.get.cancelBooking', $booking->id) }}"
                                        class="badge badge-primary badge-icon ml-1" onclick="return confirm('Are you sure you want to cancel this booking?')"> {{ __('storeDashboard.cancel')}} <i
                                            class="icon-cross"></i></a>
                                    @endif
                                    @if($booking->booking_type == 'recurring' )
                                        <a href="{{ route('restaurant.get.cancelAllBooking', $booking->id) }}"
                                        class="badge badge-primary badge-icon ml-1" onclick="return confirm('Are you sure you want to cancel all future booking?')"> {{ __('storeDashboard.cancelall')}} <i
                                            class="icon-cross"></i></a>
                                    @endif
                                    @if($booking->booking_status == 'open' || $booking->booking_status == 'reserved')
                                        <div class="checkbox checkbox-switchery ml-1" style="padding-top: 0.8rem;">
                                            <label>
                                            <input value="true" type="checkbox" class="action-switch"
                                            @if($booking->booking_status == 'completed') checked="checked" @endif data-id="{{ $booking->id }}">
                                            </label>
                                        </div>
                                    @endif
                                </div>
                ...
                <div class="mt-3">
                    {{ $bookings->appends($pagination)->links() }}
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title"><span class="font-weight-bold">{{__('storeDashboard.bpmTitle')}}</span></h5>
                <button type="button" class="close" data-dismiss="modal">&times;</button>
            </div>
            <div class="modal-body">
                <form action="{{ route('restaurant.saveNewBooking') }}" method="POST" enctype="multipart/form-data" id="strip_payment_booking">
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-lg-9">
                            <select class="form-control select-search booking_store_select" id="booking_store_select" name="restaurant_id" required>
                                ...
                                @foreach ($restaurants as $restaurant)
                                    @if($restaurant->restaurantSettings->sommelier_reservations == 'yes')
                    ...
                    <div class="form-group row">
                        ...
                        <input type="date" class="form-control form-control-lg" id="booking_date_input" name="booking_date" Placeholder="Booking Date" style="padding:.5625rem 1rem;" required>
                            <!-- <textarea class="summernote-editor" name="desc" placeholder="{{__('storeDashboard.bpmPhDescription')}}"
                                rows="6"></textarea> -->
                        </div>
                    </div>
...
<div id="addBulkItemModal" class="modal fade mt-5" tabindex="-1">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title"><span class="font-weight-bold">{{__('storeDashboard.bpmCsvTitle')}}</span></h5>
                <button type="button" class="close" data-dismiss="modal">&times;</button>
            </div>
            <div class="modal-body">
                <form action="{{ route('restaurant.itemBulkUpload') }}" method="POST" enctype="multipart/form-data">
                    <div class="form-group row">
                        <label class="col-lg-2 col-form-label">{{__('storeDashboard.bpmLabelCsvFile')}}: </label>
                        <div class="col-lg-10">
                            <div class="uploader">
                    ...
                    <div class="text-left">
                        ...
                        {{__('storeDashboard.bpmBtnCsvDownloadSample')}}
                            <i class="icon-file-download ml-1"></i>
                        </button>
                    </div>
                    <div class="text-right">
                        <button type="submit" class="btn btn-primary">
                        {{__('storeDashboard.bpmBtnCsvUpload')}}
...
<script>
        ...
        if (Array.prototype.forEach) {
         ...
         $('.action-switch').click(function(event) {
            ...
            if(choice){
                ...
                let url = "{{ url('/store-owner/booking/disable/') }}/"+id;
         ...
         $('.action-switch-booking-done-all').click(function(event) {
            ...
            if(choice){
                ...
                let url = "{{ url('/store-owner/booking/doneall/') }}/?booking_date="+booking_date+"&booking_status="+booking_status;
                window.location.href = url;
            }else{
                return false;
            }
         ...
         $('.filter_restaurant_dropdown').change(function(){
            restaurant_id = jQuery(this).val();
             if(restaurant_id != null && restaurant_id != "all"){
                $.ajax({
                    type:'post',
                    url:'{!! url('store-owner/store/shift-timing-for-filter') !!}/'+restaurant_id,
                   data:{_token: "{{ csrf_token() }}", time_slot: "{{ (!empty($time_slot)) ? $time_slot : ''}}"},
                    success:function(data) {
                        if(data.success){
         ...
         $('.booking_store_select').change(function(){
            restaurant_id = jQuery(this).val();
             if(restaurant_id != null && restaurant_id != ""){
                $.ajax({
                    type:'post',
                    url:'{!! url('store-owner/store/restaurant-table-areas') !!}/'+restaurant_id,
    ...
    $('#table_location_dropdown_addnew').change(function() {
        ...
        $.ajax({
            url: "{{ route('restaurant.get.getSettingsRestaurantbooking', ['id' => ':id']) }}".replace(':id', restaurantId), // Ensure correct route name
            method: 'GET',
        ...
        $.ajax({
            url: "{{ route('restaurant.get.getRestaurantbooking', ['id' => ':id']) }}".replace(':id', restaurantId), // Ensure correct route name
            method: 'GET',
            data: requestData, // Send data as query parameters
            success: function(response) {
