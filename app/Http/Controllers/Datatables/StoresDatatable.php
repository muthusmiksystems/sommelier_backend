    ...
    public function storesDatatable(Request $request)
    {
        ...
        return Datatables::of($restaurants)
            ...
            ->addColumn('action', function ($restaurant) use ($request) {
                ...
                $html .= '<a href="'.route('admin.get.tableShiftRestaurant', $restaurant->id).'" class="btn btn-sm btn-primary ml-1">'.__('storeDashboard.resTableShift').' </a>';
                $html .= '<a href="'.route('admin.get.settingsRestaurant', $restaurant->id).'" class="btn btn-sm btn-primary ml-1"> Settings</a>';
