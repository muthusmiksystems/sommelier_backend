...
class SettingController extends Controller
{
    ...
    public function saveSettings(Request $request, Factory $cache): RedirectResponse
    {
        // dd($request->all());
        ...
        if ($request->hasFile('firstScreenHeroImage')) {
            ...
            $setting->value = 'assets/img/various/'.$filename.'?v='.time().Str::random(5);
            $setting->save();
            $setting = Setting::where('key', 'firstScreenHeroImageSm')->first();
            $setting->value = 'assets/img/various/'.$filenameSm.'?v='.time().Str::random(5);
            $setting->save();
        }
        ...
        $env->setKey('MAIL_MAILER', $request->defaultEmailGateway);
        ...
        $versionJson = json_decode($versionJson);
        $fileData = [
            'forceNewSettingsVersion' => strtolower(Str::random(30)),
    ...
    public function saveSpecificSettings(Request $request): RedirectResponse
    {
    ...
    public function forceClear(Request $request): JsonResponse
    {
        Artisan::call('cache:clear');
        ...
        switch ($request->type) {
            case 'CACHE':
                $fileData = [
                    ...
                    'forceCacheClearVersion' => strtolower(Str::random(30)),
                ];
    ...
    public function cleanEverything(Request $request): JsonResponse
    {
        //clear all db
        ...
        try {
            ...
            DB::table('restaurant_customer_model')->where('model_id', '<>', '1')->delete();
    ...
    public function deleteJunkActivityLogs(Request $request): RedirectResponse
    {
