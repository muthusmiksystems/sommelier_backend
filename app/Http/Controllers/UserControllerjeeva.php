...
class UserControllerjeeva extends Controller
{
    /**
     * @return mixed
     */
            ...
            if ($user && $request->email === $user->email) { // The passwords match...
                ...
                if (isset($request->storeSlug)) {
                    try {
                        ...
                        $resIdValue = Restaurant::findOrFail($restaurantData->id);
                        // Retrieve the role_id for 'Customer'
                        $role_id = DB::table('roles')->where('name', 'Customer')->value('id');

                        // Check if the record already exists
                        $existingRecord = RestaurantCustomerModel::where('user_id', $user->id)
                            ->where('restaurant_id', $resIdValue)
                            ->where('role_id', $role_id)
                            ->first();
    ...
    public function register(Request $request): JsonResponse
    {
        ...
        try {
            ...
            if ($otpTable->otp == $request->otp && $user->save()) {
    ...
    public function checkRunningOrderApp(Request $request): JsonResponse
    {
